...
// ----------------------------------------------------------------------------------
//
// init_led_io() -- Initialize I/O pins for on-board R & G LEDs (PWM)
//
static void init_led_io(void)
{
    // Turn on clock gating to PortE module (red and green LEDs)
    // and PortD (blue LED). Also PortB for other stuff
    SIM_SCGC5 |= SIM_SCGC5_PORTB_MASK | SIM_SCGC5_PORTD_MASK | SIM_SCGC5_PORTE_MASK;
    SIM_SCGC6 |= SIM_SCGC6_TPM0_MASK;
    SIM_SOPT2 |= SIM_SOPT2_TPMSRC(1);

    PORTE_PCR29 = PORT_PCR_MUX(3);  // TPM2_CH2 enable on PTE29 (red)
    PORTE_PCR31 = PORT_PCR_MUX(3);  // TPM2_CH4 enable on PTE31 (green)
    PORTD_PCR5  = PORT_PCR_MUX(4);  // TPM0_CH5 enable on PTD5  (blue)
    ...
    TPM0_C5SC = TPM_CnSC_MSB_MASK | TPM_CnSC_ELSA_MASK;
